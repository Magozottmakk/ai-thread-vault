...
document.getElementById('saveBtn').addEventListener('click', async () => {
  ...
  function universalScrapeAndDownload(format) {
    ...
    a.href = url;
    // Módosított fájlnév: AI - Dátum
    a.download = `${aiName} - ${dateStr}.${format}`;
    a.click();
    URL.revokeObjectURL(url);
  }
  
  // --- Visszatöltés funkció ---
  ...
  fileInput.addEventListener('change', (event) => {
    ...
    reader.onload = async (e) => {
      ...
      if (file.name.endsWith('.json')) {
        ...
        } catch (err) { console.error("JSON hiba"); }
      }
      ...
      if (tab && tab.id) {
        chrome.tabs.sendMessage(tab.id, { action: "pasteToChat", text: finalPrompt }, (response) => {
          if (chrome.runtime.lastError || (response && response.status === "error")) {
            copyToClipboardFallback(finalPrompt);
          } else {
            window.close();
          }
        });
  ...
  async function copyToClipboardFallback(text) {
    ...
    try {
      ...
      alert("Az azonnali beillesztés ennél az AI-nál nem sikerült, de a szöveg a vágólapra került!\n\nKérlek, kattints a chatmezőbe és nyomj Ctrl+V-t a folytatáshoz.");
    } catch (err) {
      ...
      alert("Siker! (Tartalék módszerrel másolva).\n\nA tartalom beillesztéséhez használd a Ctrl+V billentyűkombinációt a chatmezőben.");
    }
  }